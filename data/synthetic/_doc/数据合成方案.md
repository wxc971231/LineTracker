...
<img src="./img/base_func.png" alt="基函数" style="zoom:67%;" />
...
第 3.1–3.10 节已经给出了完整的建模逻辑。实现拆分为两个脚本：[fit_dataset1.py](../fit_dataset1.py) 只读取真实 `dataset1` 数据并输出模型；[generate_dataset1.py](../generate_dataset1.py) 只读取该模型并批量生成合成背景。

| 文档步骤            | 主要实现                                                     | 结果                                                         |
| ------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| 3.1–3.3             | [fit_dataset1.py](../fit_dataset1.py)：[`profile_real_sample`](../fit_dataset1.py#L97)、[`fit_dataset1`](../fit_dataset1.py#L272)、[`gaussian_filter1d`](../fit_dataset1.py#L304) | 原始矩阵先二值化、屏蔽 bin 0，再得到 $\hat p_{j,k}$、$a_j(k)$ 与 $\tilde a_j(k)$。 |
| 3.4–3.6（拟合部分） | [fit_dataset1.py](../fit_dataset1.py)：[`fit_dataset1`](../fit_dataset1.py#L272) | 计算 $\bar a(k)$、三个基函数、真实样本系数和连续可调范围，并写入模型文件。 |
| 3.6                 | [generate_dataset1.py](../generate_dataset1.py)：[`step_3_6_select_coefficients`](../generate_dataset1.py#L524) | 将三个归一化距离曲线旋钮映射到模型保存的连续系数范围，得到本样本的 $\boldsymbol\theta$。 |
| 3.7                 | [generate_dataset1.py](../generate_dataset1.py)：[`step_3_7_restore_probability`](../generate_dataset1.py#L558) | 由平滑真实曲线现场构建 $a$ 空间包络；用 $\boldsymbol\theta$ 构造、裁剪并恢复 Bernoulli 概率，插值后得到 $p_{\boldsymbol\theta}(r)$。 |
| 3.8                 | [generate_dataset1.py](../generate_dataset1.py)：[`step_3_8_prepare_spatial_clusters`](../generate_dataset1.py#L598) | 根据 `cluster` 标定 $s,e_1,e_2,e_3$，并求出基准种子概率 $q(r)$。 |
| 3.9                 | [generate_dataset1.py](../generate_dataset1.py)：[`step_3_9_prepare_frame_adjustment`](../generate_dataset1.py#L705) | 根据 `gain` 与目标 Fano 生成 $c_t$，并准备按帧块计算 $q_t(r)$。 |
| 3.10                | [generate_dataset1.py](../generate_dataset1.py)：[`step_3_10_generate_binary_background`](../generate_dataset1.py#L736) | 按 Bernoulli 规则采样种子，以共享随机数进行 1–3 m 嵌套扩展，并用逻辑或得到 $B_{t,r}$。[`generate_batch`](../generate_dataset1.py#L963) 负责循环生成、保存文件与批次清单。 |
