...
- **空间分块**：距离块宽度 $W_R$ 和步长 $S_R$ 均可按实现资源自由设置，只需满足
    ...
    这时唯一的距离块就是完整 $0\sim R-1$ 距离轴。**若 $M>1$，应令 $O_R$ 不小于单个时间窗内目标可能跨越的最大距离，以保证完整航迹包含在至少一个距离块中**。以 $v_{\max}=340\,\mathrm{m/s}$ 和完整 1 s 窗口边界计算时，$340\,\mathrm{m}$ 是保守重叠裕量；对 20 个离散采样帧，候选线首末采样位置的最大跨度约为 $324\,\mathrm{m}$。因此第一版可取 $O_R\ge340\,\mathrm{m}$。例如 $W_R=10\,\mathrm{km}$、$S_R=9\,\mathrm{km}$、$O_R=1\,\mathrm{km}$ 是安全的多块处理配置。
...
| 符号 | 含义 |
|---|---|
| $F_{\mathrm{line}}$ | 单条线匹配的时间窗长度，第一版固定为 20 帧 |
...
| $\hat\pi_w^{(n)}(r)$ | 第 $n$ 个时间窗用于显著性计算的池化背景概率估计 |
...
| $\kappa_{\mathrm{bg}}$ | 背景方差膨胀系数；补偿帧级共同波动和残余时间相关 |
| $\omega_{k,\tau}$ | CNN 或动态规划给出的逐帧置信度 |
| $\gamma_k^{\mathrm{refine}}$ | 细化器给出的整条候选置信度；可来自 CNN 或动态规划 |
...
X_{t,r+\delta}.
$$
全局距离范围外的 $X_{t,r}$ 视为 0。它表示候选线在某帧附近只要出现至少一个响应就计为一次命中；同一帧内，距离窄带内的多个背景响应不会重复计分。

实际按距离块处理时，每个块除读取自身的 $W_R$ 个 bin 外，还应额外读取左右各 $w_{\mathrm{pool}}$ m 的 halo：

$$
d=-w_{\mathrm{pool}},\ldots,W_R-1+w_{\mathrm{pool}}.
$$

利用扩展读入的数据计算池化响应后，只保留本地中心位置 $d=0,\ldots,W_R-1$ 的 $Y^{(w)}$ 参与该块匹配。这样块边缘的池化结果与先在完整距离轴上计算 $Y^{(w)}$ 再切块完全一致。
...
其中 $|\mathcal R_k|=1000$、$F_{\mathrm{line}}=20$，$t_n$ 是第 $n$ 个时间窗的起始帧，$Y^{(w)}_{t,r}\in\{0,1\}$ 表示第 $t$ 帧、以 $r$ 为中心的窄带内是否至少有一个响应。

对 300 个 $\bar\pi^{\mathrm{obs},(n)}_{w,k}$ 做截尾损失拟合或 Huber 平滑，并插值回 1 m 距离轴，得到：

$$
\tilde\pi^{\mathrm{obs},(n)}_w(r)
=
...
\hat\pi_w^{(n)}(r)
=
\alpha(r)\tilde\pi^{\mathrm{obs},(n)}_w(r)
+
...
第一版可设常数 $\alpha(r)=0.7$，即当前时间窗统计为主、参考曲线为辅。远场若需要更稳定，可降低 $\alpha(r)$；若场景与训练背景差异很大，或没有可信参考曲线，可令 $\alpha(r)=1$，完全使用当前时间窗的观测估计。


### 3.4 候选遮罩重估计
候选遮罩重估计是一个可选的第二阶段，发生在**粗匹配已经得到候选直线之后、最终筛选之前**；它不是在尚未知道候选线位置时预先进行。完整流程如下：

1. **初始粗匹配：** 用未遮罩数据估计的初始 $\hat\pi_w^{(n)}(r)$，对全部 $(\nu_j,\rho)$ 计算沿线池化命中数 $A^{(n,m)}(j,\rho)$ 和初始显著性得分 $Z^{(n,m)}(j,\rho)$，选出 Top-$K$ 条候选直线。
2. **按候选线遮罩：** 对每一条 Top-$K$ 候选线及每个局部帧 $\tau$，将其预测距离附近 $\pm w_{\mathrm{mask}}$ m 的时距单元暂时标记为不参与背景统计。重算 $\bar\pi^{\mathrm{obs},(n)}_{w,k}$ 时，这些单元应同时从分子和分母中排除。
3. **重估背景：** 仅用剩余单元重新得到 $\tilde\pi^{\mathrm{obs},(n)}_w(r)$，并与 $\pi_w^{\mathrm{ref}}(r)$ 融合为新的 $\hat\pi_w^{(n)}(r)$。
...
- **偏置**：使用考察时间-距离窗口中心时刻 $\tau_0=\frac{F_{\mathrm{line}}-1}{2}$ 的候选直线目标距离 $\rho$ 表示直线偏置。粗搜索在可行的参考距离中按步长 $s_{\rho}^{\mathrm{coarse}}$ 取样：
    ...
    0\le\rho+\Delta_{j,\tau}\le W_R-1,\ 
    \forall\tau\in\{0,\ldots,F_{\mathrm{line}}-1\}
    \right\}.
    $$

    这里的边界条件保证候选线的每个中心位置均落在本距离块内；halo 只用于计算这些中心位置的窄带池化值，并不允许候选线主体越过块边界。因此对每个斜率的参考距离候选数由约 $W_R$ 个降为至多 $\lceil W_R/s_{\rho}^{\mathrm{coarse}}\rceil$ 个。第一版可取
...
于是，第 $m$ 个距离块中 $(j,\rho)$ 对应候选线在第 $\tau$ 帧的全局预测距离为：
...
### 4.2 背景显著性
若直接按候选直线的池化命中计数 $A^{(n,m)}(j,\rho)$ 排序，近场高背景会产生大量伪峰。因此，先假设候选线附近没有目标，只有纯背景噪声。此时 $\hat\pi_w^{(n)}\!\left(r^{(m)}_{j,\rho}(\tau)\right)$ 表示第 $n$ 个时间窗中、第 $\tau$ 帧在该候选线预测位置附近仅由背景产生一次池化命中的概率，即有
$$
Y^{(w)}_{t_n+\tau,\,r^{(m)}_{j,\rho}(\tau)}
\sim
\operatorname{Bernoulli}\Big(\hat\pi_w^{(n)}\!(r^{(m)}_{j,\rho}(\tau))\Big),
$$
...
\right].\\
&=\sum_{\tau=0}^{F_{\mathrm{line}}-1}
\hat\pi_w^{(n)}\!\left(r^{(m)}_{j,\rho}(\tau)\right).
\end{aligned}
$$
...
- 再定义 $\left[\sigma^{(n,m)}(j,\rho)\right]^2$ 为同一**纯背景条件下**候选线池化命中计数 $A^{(n,m)}(j,\rho)$ 的方差。由于命中次数
    ...
    是一系列 Bernoulli 随机变量的和，若逐帧池化命中在纯背景条件下独立，则独立近似方差为
    ...
    \sum_{\tau=0}^{F_{\mathrm{line}}-1}
    \hat\pi_w^{(n)}\!\left(r^{(m)}_{j,\rho}(\tau)\right)
    \left[1-\hat\pi_w^{(n)}\!\left(r^{(m)}_{j,\rho}(\tau)\right)\right].
    $$

    实际背景可能仍有帧级亮暗波动或残余时间相关，因此用方差膨胀系数作保守近似：

    $$
    \left[\sigma^{(n,m)}(j,\rho)\right]^2
    \approx
    \kappa_{\mathrm{bg}}V_{\mathrm{ind}},
    \qquad
    \kappa_{\mathrm{bg}}\ge1.
    $$

    当 $\kappa_{\mathrm{bg}}=1$ 时，才是严格按帧间独立 Bernoulli 近似处理；$\kappa_{\mathrm{bg}}>1$ 用于吸收未显式建模的协方差和帧级波动。
...
对任意宽度为 $W_R$ 的距离块，移位—累加是最直接的实现。对每个 $\nu_j$，按 $\Delta_{j,\tau}$ 读取 20 帧的池化结果并相加；粗阶段只在 $\mathcal P^{\mathrm{coarse}}_{m,j}$ 中的参考距离位置输出 $A^{(n,m)}(j,\rho)$。令

$$
N_{\rho,m,j}^{\mathrm{coarse}}
=
\left|\mathcal P^{\mathrm{coarse}}_{m,j}\right|
\le
\left\lceil\frac{W_R}{s_{\rho}^{\mathrm{coarse}}}\right\rceil,
$$
...
~~~
池化输入： (20, W_R)
匹配计数： (69, ceil(W_R / s_rho^coarse))
显著性图： (69, ceil(W_R / s_rho^coarse))
~~~

例如保留原来的 $W_R=10000\,\mathrm{m}$ 块宽，并取 $s_{\rho}^{\mathrm{coarse}}=4\,\mathrm{m}$ 时：
...
对每个粗候选的细搜只在其附近数米范围内按 $s_{\rho}^{\mathrm{fine}}=1\,\mathrm{m}$ 计算，不再形成整块的 1 m 全距离得分图。

若输入足够稀疏，也可以将所有 $Y^{(w)}_{t,r}=1$ 的**池化命中位置**表示为 $(\tau,r_i)$。对于每个命中位置及每个斜率，计算：
...
若 $\rho_{i,j}\in\mathcal P^{\mathrm{coarse}}_{m,j}$，再执行：
...
对每个粗显著性图 $Z^{(n,m)}$ 执行二维 NMS，同时抑制距离接近与斜率接近的重复峰值。每个距离块先保留：
...
当 $M>1$ 时，将所有重叠块的候选映射到全局坐标后，再执行一次全局 NMS。若两条候选在同一 20 帧窗口中的预测距离中位差较小，且斜率接近，则视为同一航迹的重复候选，只保留 $Z_k$ 更高者。这样保留 $O_R$ 重叠带来的边界召回率，又避免重复送入后续模块；单块模式 $M=1$ 时跳过这一步。
...
BSN-MSLF 的职责是高召回候选生成，而不是独立完成最终逐帧坐标恢复。设细筛后的第 $k$ 条候选来自距离块 $m_k$，其细化后的中心距离和斜率分别为 $\tilde\rho_k$、$\tilde\nu_k$；若没有执行细搜，它们就取对应粗候选的 $\rho_k$、$\nu_k$。定义其全局粗线为：
...
的 batch 输入 CNN。靠近全局距离边界时，可将越界的裁剪位置按 $X_{t,r}=0$ 补齐。CNN 只需预测相对粗线的逐帧修正：
...
也可以先不用 CNN，而在候选窄带中使用动态规划恢复逐帧路径，并由路径代价或路径间隔构造 $\omega_{k,\tau}$ 与 $\gamma_k^{\mathrm{refine}}$。无论采用何种细化器，最终均可对可信坐标进行加权直线或二次曲线拟合：
...
| 线匹配时间窗 $F_{\mathrm{line}}$ | 20 帧 | 覆盖 1 s；首末采样时刻相隔 0.95 s，当前微曲数据在此尺度内近似直线 |
| 距离块宽度 $W_R$ | 可配；多块示例为 10 km | 可取 $W_R=R$ 作为单块模式 |
| 距离块步长 $S_R$ | 可配；多块示例为 9 km | 单块模式取 $S_R=R$ |
| 距离块重叠 $O_R$ | 多块时至少 340 m；示例为 1 km | 只在 $M>1$ 时需要；保证完整候选线至少落入一个块 |
...
| 方差膨胀系数 $\kappa_{\mathrm{bg}}$ | 由无目标背景标定，且 $\ge1$ | 用于使显著性方差覆盖帧级共同波动和残余时间相关 |
...
所有 baseline 应使用完全相同的时间窗、距离分块、halo、斜率网格、粗/细参考距离网格和候选去重协议。当前多块示例为 $20\times10\,\mathrm{km}$、1 km 重叠；若采用单块模式，则统一设置 $W_R=S_R=R$、$O_R=0$，且不执行跨块去重。

1. 原始 Hough：直接按投票数排序；
2. MSLF：池化后的多斜率匹配，仅使用 $A^{(n,m)}(j,\rho)$；
3. BSN-MSLF：使用自适应 $\hat\pi_w^{(n)}(r)$ 与显著性得分 $Z^{(n,m)}(j,\rho)$；
...
- 候选遮罩重估计前后的排名变化；
- 单个 $20\times W_R$ 块及完整冷启动搜索耗时；其中应单独报告 $W_R=10\,\mathrm{km}$ 多块示例的结果。
