...
立即生成一条斜率为 $\nu_t^{\mathrm{cap}}$、过 $r_t^{\mathrm{cap}}$ 点的综合预测直线，并向未来外推 $S_T^{\mathrm{track}}$ 帧，得到 TRACK 模式第一个时间步进内的目标位置预测
...
| -------------------- | --------------------------------- |
| $L=0$                | 中心 $W_R$ 块，共1块              |
| $L=1$                | 中心块加左右各1个 $W_R$ 块，通常共3块 |
| $L=2$                | 中心块加左右各2个 $W_R$ 块，通常共5块 |
...
靠近 $0$ 或 $300\ \mathrm{km}$ 测距边界时，多个偏移后的块起点可能被裁剪到相同位置；实现会对这些重复块去重，因此实际前向块数可能少于表中的通常数量。
...
| $q_{\mathrm{cap}}$ | $0.5$ | `capture_q_min`：CAPTURE/RECAPTURE 的全局 q-Top1 写入捕获缓存所需的最低分类分数。 |
