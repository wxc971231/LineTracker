...
def get_config() -> SimpleCNNConfig:
    ...
    return SimpleCNNConfig(
        ...
        model_type="s",                         # 模型容量：n 为原始基线；可通过 --set model_type=s 切换到 1.5 倍宽度的 s。
        ...
        learning_rate=5e-4,                     # AdamW 的峰值学习率；warmup 后从该值开始余弦衰减。
        warmup_ratio=0.03,                      # 前 5% optimizer steps 线性 warmup。
